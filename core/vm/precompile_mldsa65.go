...
// Provisional gas schedule (tune with benchmarks later)
const (
	mldsa65GasBase    uint64 = 250_000
	mldsa65GasPerWord uint64 = 12 // per 32-byte word of msg
)

type mldsa65VerifyPrecompile struct{}

func (p *mldsa65VerifyPrecompile) RequiredGas(input []byte) uint64 {
	// msgLen = max(0, len(input) - pkLen - sigLen)
...
func (p *mldsa65VerifyPrecompile) Run(input []byte) ([]byte, error) {
	...
	// UnmarshalBinary validates length/structure
	if err := pk.UnmarshalBinary(pkBytes); err != nil {
		return zero, nil
	}

	// Context is nil/empty; verify msg as provided
